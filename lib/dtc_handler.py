...
def dtcs_to_human(dtcs):
    ...
    # Check if CSV file exists, if not fall back to raw output
    if not os.path.exists(csv_path):
        for dtc in dtcs:
            ...
            dtc_desc = f"DTC {dtc_id} Status: {','.join(dtc_state)}"
        ...
        for dtc in dtcs:
            ...
            for row in csv_data:
                if int(row["code"]) == int(dtc_id):
                    dtc_desc = f"{dtc_id} : {row['pcode']} : {row['name']}, {row['symbol']} Status: "
                    dtc_state = []
                    if dtc_status.test_failed:
                        dtc_state.append("Test Failed")
                    if dtc_status.test_failed_this_operation_cycle:
